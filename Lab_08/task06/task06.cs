...
namespace Lab08
{
    public partial class task06 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < level1Items.Length; i++)
            {
                foreach (string level2 in level2Items[i])
                {
                    ...
                    level2Item.Click += MenuItem_Click;
            ...
            for (int i = 0; i < level3Items.Length; i++)
            {
                foreach (string level3 in level3Items[i])
                {
                    ...
                    level3Item.Click += MenuItem_Click;
        ...
        // Єдиний обробник подій для всіх кінцевих пунктів меню
        private void MenuItem_Click(object sender, EventArgs e)
        {
            // Визначаємо, який пункт меню викликав подію
            if (sender is ToolStripMenuItem clickedItem)
            {
                MessageBox.Show("Ви вибрали: " + clickedItem.Text);
            }
        }
    }
}
